...
const DEFAULT_IDENTITY_SCRUB = [
    // Claude Code / Anthropic — Claude Code's system prompts identify as
    // "Claude, made by Anthropic", which trips cloudcode-pa's third-party AI
    // filter (issue #375, Trigger #2). Order matters: the more specific phrase
    // must be replaced before the shorter substring it contains.
...
function scrubClientIdentity(text) {
    ...
    let out = text;
    // Defensive second layer: strip Anthropic telemetry / billing pseudo-headers.
    // The primary strip happens in request-converter.cleanSystemInstructionText, but
    // we repeat it here so any system text reaching the Cloud Code payload is clean
    // even if it arrived through a path that bypassed the converter. cloudcode-pa
    // returns an opaque 429 when it sees the `x-anthropic-*` header string.
    out = out
...
export function buildCloudCodeRequest(anthropicRequest, projectId, accountEmail) {
    ...
    // third-party AI product identities and telemetry/billing pseudo-headers that
    // trip cloudcode-pa's 429 (see above). Skip parts that become empty after
    // scrubbing so we never forward a blank part.
    if (googleRequest.systemInstruction && googleRequest.systemInstruction.parts) {
        for (const part of googleRequest.systemInstruction.parts) {
            if (part.text) {
                const scrubbed = scrubClientIdentity(part.text);
                if (scrubbed && scrubbed.trim().length > 0) {
                    systemParts.push({ text: scrubbed });
                }
            }
        }
    }
